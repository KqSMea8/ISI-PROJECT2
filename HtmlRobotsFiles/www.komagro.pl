...
<script> var wersja_strony= '1280';if(wersja_strony==='auto'){$.ajax({type:'GET',url:'/komponenty/ajax/session.php?font_size='+font_size+'&kontrast='+kontrast+'&wersja_strony='+screen.width,success:function(){location.href='http://www.komagro.pl/robots.txt/'}});}</script>
...
location.href='http://www.komagro.pl/robots.txt/';
}
}});
}
